...
(function () {
  ...
  // Botón "Lo mejor para ti": sin sesión -> crear perfil; con sesión -> recomendaciones.
  // hero.js se carga antes que supabase-config.js, así que esperamos a 'load'.
  function wireMejor() {
    var b = document.getElementById("btn-mejor");
    if (!b) return;
    function setHref() {
      if (!window.sb) { b.setAttribute("href", "cuenta.html"); return; }
      window.sb.auth.getSession().then(function (r) {
        b.setAttribute("href", r && r.data && r.data.session ? "recomendadas.html" : "cuenta.html");
      }).catch(function () { b.setAttribute("href", "cuenta.html"); });
    }
    if (window.sb) setHref();
    else window.addEventListener("load", setHref);
  }
